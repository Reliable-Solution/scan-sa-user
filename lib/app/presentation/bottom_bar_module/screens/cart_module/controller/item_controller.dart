...
class ItemController extends GetxController implements GetxService {
  ...
  Future<void> getFeaturedCategoriesItemList(
    ...
    if (_featuredCategoriesItem == null || reload || fromRecall) {
      if (dataSource == DataSourceEnum.local) {
        ...
        update();
        await getFeaturedCategoriesItemList(
          false,
          notify,
          dataSource: DataSourceEnum.client,
          fromRecall: true,
  ...
  Future<void> getConditionsWiseItem(int id, bool notify) async {
    ...
    final items = await itemServiceInterface.getConditionsWiseItems(id);
  ...
  Future<void> getCommonConditions(bool notify) async {
    ...
    final conditions = await itemServiceInterface.getCommonConditions();
  ...
  Future<int> setExistInCart(
    ...
    final variationType = await itemServiceInterface.prepareVariationType(
      item!.choiceOptions,
      _variationIndex,
    );
  ...
  //       showCartSnackBar();
  //     }
  //   } else if (Get.find<GlobalController>()
  //           .configModel!
  //           .moduleConfig!
  //           .module!
  //           .showRestaurantText! ||
