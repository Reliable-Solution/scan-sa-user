...
class SearchRepository implements SearchRepositoryInterface {
  ...
  Future<List<Item>?> _getSuggestedItems() async {
    ...
    if (response.statusCode == 200) {
      ...
      response.body.forEach(
        (suggestedItem) => suggestedItemList!.add(
          Item.fromJson(suggestedItem as Map<String, dynamic>),
        ),
      );
    }
  ...
  Future<SearchSuggestionModel?> getSearchSuggestions(String searchText) async {
    ...
    if (response.statusCode == 200) {
      searchSuggestionModel = SearchSuggestionModel.fromJson(
  ...
  Future<List<PopularCategoryModel?>?> getPopularCategories() async {
    ...
    if (response.statusCode == 200) {
      ...
      response.body.forEach((category) {
        popularCategoryList!.add(
          PopularCategoryModel.fromJson(category as Map<String, dynamic>),
        );
      });
    }
