...
class SearchController extends GetxController implements GetxService {
  SearchController(
    ...
    searchType.value = context.l10n.restaurants;
  }
  ...
  Future<void> getSuggestedItems() async {
    final suggestedItemList =
        await searchServiceInterface.getSuggestedItems() as List<Item>;
    _suggestedItemList = [];
  ...
  Future<void> searchData(String? query, bool fromHome) async {
    if ((_isStore && query!.isNotEmpty && query != _storeResultText) ||
      ...
      isSearchLoad = true;
      update();
      await searchServiceInterface.saveSearchHistory(_historyList);
      _isSearchMode = false;
      if (!fromHome) {
        update();
      }
      ('object === object').print;
      final response = await searchServiceInterface.getSearchData(
        query,
        _isStore,
      );
  ...
  Future<void> clearSearchHistory() async {
    await searchServiceInterface.clearSearchHistory();
    _historyList = [];
    update();
  }
