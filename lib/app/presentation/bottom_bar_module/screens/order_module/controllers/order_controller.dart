...
class OrderController extends GetxController implements GetxService {
  ...
  Future<ResponseModel?> trackOrder(
    ...
    if (orderModel == null) {
      ...
      if (response.statusCode == 200) {
        _trackModel = OrderModel.fromJson(
  ...
  Future<ResponseModel?> timerTrackOrder(
    ...
    if (response.statusCode == 200) {
      _trackModel = OrderModel.fromJson(response.body as Map<String, dynamic>);
      _responseModel = ResponseModel(true, response.body.toString());
    } else {
      _responseModel = ResponseModel(false, response.statusText);
    }
  ...
  Future<bool> cancelOrder(
    ...
    _isLoading = true;
    update();
    final success = await orderServiceInterface.cancelOrder(
      orderID.toString(),
      cancelReason,
      guestId: guestId,
    );
  ...
  Future<bool> switchToCOD(String? orderID, {String? guestId}) async {
    _isLoading = true;
    update();
    final isSuccess = await orderServiceInterface.switchToCOD(
      orderID,
      guestId: guestId,
    );
    _isLoading = false;
