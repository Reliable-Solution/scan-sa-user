...
class StoreRepository implements StoreRepositoryInterface {
  ...
  Future<List<MenuModelData>?> getStoreMenuList({
    ...
    List<MenuModelData>? storeMenuList;
    String cacheId = '${AppConstants.getStoreMenuList}/?store-id=storeId';

    switch (source) {
      case DataSourceEnum.client:
        Response response =
            await apiClient.getData(AppConstants.getStoreMenuList);
        if (response.statusCode == 200) {
          storeMenuList = [];
          final data = response.body as Map<String, dynamic>;
          for (var store in (data['data'] as List<Map<String, dynamic>>)) {
            storeMenuList.add(MenuModelData.fromJson(store));
          }
          LocalClient.organize(
            DataSourceEnum.client,
            cacheId,
      ...
      case DataSourceEnum.local:
        ...
        if (cacheResponseData != null) {
          storeMenuList = [];
          jsonDecode(cacheResponseData).forEach(
            (store) => storeMenuList!.add(MenuModelData.fromJson(store)),
          );
        }
    }
