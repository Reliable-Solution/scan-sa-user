...
import 'package:scan_sa_user/features/store/domain/models/cart_suggested_item_model.dart';
...
import 'package:scan_sa_user/features/store/domain/models/recommended_product_model.dart';
import 'package:scan_sa_user/features/store/domain/models/store_banner_model.dart';
...
class StoreController extends GetxController implements GetxService {
  ...
  List<MenuModelData> _storeMenuList = [];
  List<MenuModelData> get storeMenuList => _storeMenuList;
  ...
  Future<void> getFeaturedStoreList({
    ...
    print("====>>> getFeaturedStoreList == here ");
    List<Store>? stores;
    if (dataSource == DataSourceEnum.local) {
      stores =
  ...
  _prepareFeaturedStore(List<Store>? stores) {
    print("====>>> getFeaturedStoreList == here 2 == ${stores?.length}");
    if (stores != null) {
      _featuredStoreList = [];
  ...
  ValueNotifier<bool> isMenuLoad = ValueNotifier(false);
  Future<void> getStoreMenuList({
    DataSourceEnum dataSource = DataSourceEnum.local,
  }) async {
    ...
    _storeMenuList = [
      MenuModelData(
        imageUrl:
            "https://imgs.search.brave.com/hxL7QfBRjKdgMmasfWyUo2OT1xjcTPoOwaHwScigW3c/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9tYXJr/ZXRwbGFjZS5jYW52/YS5jb20vRUFHTGRO/aFVFc3cvMS8wLzEx/MzF3L2NhbnZhLW9y/YW5nZS1hbmQtYmxh/Y2stbW9kZXJuLWZv/b2QtbWVudS1tZW51/LWtKcjVzZWdTNkFj/LmpwZw",
      ),
      MenuModelData(
        imageUrl:
            "https://imgs.search.brave.com/Y3F0hE8fw-AFBVu83salYmsxqFdrAqHtXjG4EStvMG4/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9pLnBp/bmltZy5jb20vb3Jp/Z2luYWxzL2M4LzM2/L2RkL2M4MzZkZGZj/YTYwOGJmOGQ3MDk5/YjQ0OWUwZjI5Nzhh/LmpwZw",
      ),
    ];

    // List<MenuModelData>? menuList;
    // if (dataSource == DataSourceEnum.local) {
    //   menuList = await storeServiceInterface.getStoreMenuList(
    //     source: dataSource,
    //     storeId: store?.id ?? 0,
    //   );
    //   _prepareStoreMenu(menuList);
    //   getFeaturedStoreList(dataSource: DataSourceEnum.client);
    // } else {
    //   menuList = await storeServiceInterface.getStoreMenuList(
    //     source: dataSource,
