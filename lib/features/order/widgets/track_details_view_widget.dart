...
import 'package:scan_sa_user/common/widgets/custom_divider.dart';
...
import 'package:scan_sa_user/features/order/widgets/address_details_widget.dart';
...
class TrackDetailsViewWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool dineIn = track.orderType == 'dine_in';
    ...
    return Container(
      ...
      child: (!takeAway && !dineIn && track.deliveryMan == null)
          ? Padding(
              padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
              child: Text(
                ...
                Row(
                  children: [
                    ClipOval(
                      child: CustomImage(
                        image:
                            '${(takeAway || dineIn) ? (track.store != null ? track.store!.logoFullUrl : '') : track.deliveryMan!.imageFullUrl}',
                        height: 35,
                        width: 35,
                        fit: BoxFit.cover,
                      ),
                    ...
                    InkWell(
                      onTap: () async {
                        if (await canLaunchUrlString(
                          'tel:${(takeAway || dineIn) ? track.store != null ? track.store!.phone : '' : track.deliveryMan!.phone}',
                        )) {
                          launchUrlString(
                            'tel:${(takeAway || dineIn) ? track.store != null ? track.store!.phone : '' : track.deliveryMan!.phone}',
                            mode: LaunchMode.externalApplication,
                          ...
                          showCustomSnackBar(
                            '${'can_not_launch'.tr} ${(takeAway || dineIn) ? track.store != null ? track.store!.phone : '' : track.deliveryMan!.phone}',
                          );
                        }
                      },
                      child: Container(
