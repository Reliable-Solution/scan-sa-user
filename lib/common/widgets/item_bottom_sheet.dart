...
import 'package:scan_sa_user/common/widgets/quantity_button.dart';
    ...
    return Container(
      ...
      child: GetBuilder<ItemController>(
        builder: (itemController) {
          num? startingPrice;
          num? endingPrice;
          if ((widget.item?.choiceOptions?.isNotEmpty ?? false) &&
              (widget.item?.foodVariations?.isEmpty ?? true) &&
              !_newVariation) {
            List<num?> priceList = [];
          ...
          if (_newVariation) {
            for (int index = 0;
              ...
              for (int i = 0;
                  i <
                      (widget.item!.foodVariations![index].variationValues
                              ?.length ??
                          0);
                  i++) {
          ...
          for (int index = 0;
              index < (widget.item!.addOns?.length ?? 0);
              index++) {
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    ///Bottom side..
                    (!(widget.item?.scheduleOrder ?? false) && !isAvailable)
                        ? const SizedBox()
                        : Container(
                            decoration: BoxDecoration(
                              color: Theme.of(context).cardColor,
  ...
  Widget build(BuildContext context) {
    return item!.foodVariations != null
        ? ListView.builder(
            ...
            itemBuilder: (context, index) {
              int selectedCount = 0;
              if (item!.foodVariations![index].required ?? false) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  color: itemController.selectedVariations[index].contains(true)
                      ? context.color.secondary.withValues(alpha: 0.01)
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: Dimensions.paddingSizeExtraSmall),
                    (item?.foodVariations?[index].multiSelect ?? false)
                        ? Text(
